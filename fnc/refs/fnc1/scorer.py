...
def score_submission(gold_labels, test_labels):
    ...
    for i, (g, t) in enumerate(zip(gold_labels, test_labels)):
        ...
        y_gold_list.append(g['Stance'])
        y_test_list.append(t['Stance'])

        cm[LABELS.index(g_stance)][LABELS.index(t_stance)] += 1

    F1_scores =calculate_F1_scores(y_gold_list, y_test_list)

    return score, cm, F1_scores
...
if __name__ == '__main__':
    ...
    try:
        ...
        test_score, cm, f1_score = score_submission(gold_labels, test_labels)
        null_score, max_score = score_defaults(gold_labels)
        print_confusion_matrix(cm)
        print(SCORE_REPORT.format(max_score, null_score, test_score))
        print("Relative FNC: {:.3f}%".format((test_score / max_score)*100)+ "\n")
        print(f1_score)
